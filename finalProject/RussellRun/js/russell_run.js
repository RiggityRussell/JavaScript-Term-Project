...
var button = document.getElementById('start_button')
    button.addEventListener('click',hide_show,false);
...
function startGame() {
    ...
    yaySound = new sound("media/yay.mp3")
    woohooSound = new sound("media/woohoo.mp3")
...
function updateGameArea() {
    ...
    if (myGamePiece.crashWith(myObstacles[i])) {
        ...
        myMusic.stop();
        myGameArea.clear();
        window.alert(myScore.text)
        location.reload()
        return;
    }
    // else if (ball.crashWith(myObstacles[i])) {
    //     woohooSound.play();
    //     //return;
    //     }
    }
    ...
    myObstacles.push(new component(100, 800 - height - gap, "media/eat.png", x, height + gap,'image'));
    }
    for (i = 0; i < myObstacles.length; i += 1) {
    myObstacles[i].x += -1;
    ...
    document.addEventListener("keyup", (e) => {  // Listen for Key Press, this way we arent creating a stream of dodgeballs
        if (e.code == "Space") { // if the key pressed is space, we make a ball and throw it.
            throwBall();
        }
    })
